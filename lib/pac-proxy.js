...
PACProxy.prototype.init = function() {
	...
	downloader.get(pacfile,function(err,script){
		if (err) {
			...
			// TODO: Maybe refactor this into another async method or module?
			var sandbox =  {
			...
			var fn = vm.runInNewContext(script + ';FindProxyForURL', sandbox);
			self.findProxyForURL = fn;
			
			winston.info('PAC file %s has been received and loaded', pacfile);
...
PACProxy.prototype.handleConnect = function(req, reqSocket, bodyhead) {
	...
	proxySocket.on('connect', function () {
		...
		winston.info('CONNECT %s for %s via %s:%s from %s',req.method, req.url, pac.host, pac.port ,req.connection.remoteAddress);
